...
async function update(id, data) {
    const sql = `
        UPDATE
            evento
        SET
            nome = ?, descricao = ?, data_hora = ?, urlsiteoficial = ?
        WHERE
            id = ?
    `;
    ...
    const { nome, descricao, data_hora, urlsiteoficial } = data;

    const { changes } = await db.run(sql, [nome, descricao, data_hora, urlsiteoficial, id]);
